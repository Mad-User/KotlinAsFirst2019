...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((key, value) in grades) {
        if (!new.contains(value)) new += value to mutableListOf(key)
        else new[value]?.add(key)
    }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    val lowerCaseList = mutableListOf<Char>()
    for (char in chars) lowerCaseList.add(char.toLowerCase())

    for (item in word)
        if (lowerCaseList.contains(item.toLowerCase())) sum++
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (element in list) {
        mid = list.count { it == element }
        if (mid > 1) result[element] = mid
    }
    ...
    for (firstIndex in list.indices) {
        for (secondIndex in list.indices) {
            if (firstIndex != secondIndex) {
                if (list[firstIndex] + list[secondIndex] == number) {
